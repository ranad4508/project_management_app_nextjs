...
export class ProjectController {
  ...
  createProject = asyncHandler(async (req: NextRequest): Promise<NextResponse<ApiResponse>> => {
    const user = await requireAuth(req)
    const body = await req.json()
    const validatedData = validateSchema(schemas.createProject, body)

    const project = await this.projectService.createProject(user.id, validatedData)
  ...
  getWorkspaceProjects = asyncHandler(
    async (req: NextRequest, { params }: { params: { workspaceId: string } }): Promise<NextResponse<ApiResponse>> => {
      ...
      const result = await this.projectService.getWorkspaceProjects(workspaceId, user.id, pagination, filters)
  ...
  getUserProjects = asyncHandler(async (req: NextRequest): Promise<NextResponse<ApiResponse>> => {
    const user = await requireAuth(req)
    const { searchParams } = new URL(req.url)
    ...
    const result = await this.projectService.getUserProjects(user.id, pagination)
