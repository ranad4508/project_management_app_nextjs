...
export class NotificationService {
   ...
   */
  async getUserNotifications(userId: string, pagination: PaginationParams = {}, status?: NotificationStatus) {
   ...
   */
  async notifyWorkspaceAdmins(workspaceId: string, title: string, message: string) {
    ...
    await Promise.all(
      uniqueAdminIds.map((adminId) =>
        this.createNotification(adminId, NotificationType.WORKSPACE_UPDATED, title, message, {
          model: "Workspace",
          id: workspaceId,
        }),
      ),
   ...
   */
  async notifyProjectMembers(projectId: string, title: string, message: string, memberIds: string[]) {
    await Promise.all(
      memberIds.map((memberId) =>
        this.createNotification(memberId, NotificationType.PROJECT_UPDATED, title, message, {
          model: "Project",
          id: projectId,
        }),
      ),
