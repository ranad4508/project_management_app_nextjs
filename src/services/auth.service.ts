...
import { AuthenticationError, ConflictError, NotFoundError, ValidationError } from "@/src/errors/AppError"
import type { RegisterData, LoginCredentials, PasswordResetData, AuthUser } from "@/src/types/auth.types"
...
export class AuthService {
   ...
   * Register a new user
   */
  async register(data: RegisterData): Promise<{ user: AuthUser; message: string }> {
   ...
   * Login user
   */
  async login(credentials: LoginCredentials): Promise<{ user: AuthUser; requiresMfa?: boolean }> {
    ...
    if (user.mfaEnabled) {
      if (!mfaCode) {
        // Send MFA code
        const code = CryptoUtils.generateMfaCode()
   ...
   * Enable MFA
   */
  async enableMfa(userId: string): Promise<{ secret: string; message: string }> {
    const user = await User.findById(userId)
    if (!user) {
      throw new NotFoundError("User not found")
    }

    const mfaSecret = CryptoUtils.generateSecretKey()
    ...
    return {
      secret: mfaSecret,
      message: "MFA setup initiated. Please check your email for the verification code.",
    }
  }
